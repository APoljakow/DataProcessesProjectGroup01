...
### Abstract
The intention of this paper is to find a way to influence the incident rate related to guns. As the biggest amount of data available considering this topic comes from USA this was the chosen country to work with. Four datasets were considered: one with incidents happened from 2013-18, evolution of laws considering gun regulation, gun license acquirement and population distribution in USA. 
To gain information the incidents were related to an amount of people to normalize the value. A metric which shows how many laws are applied to gun regulation in a state and their development over the years is introduced and explained. 
  The visualized graphs and the applied model show a clear relationship between the number of gun-regulating laws which are active in a state and the proportion of killed people respect to the total population of the state.
  Considering the relationship is can or cannot be advised to apply more laws, change regulation considering the aquirement of guns and a govermental controlled instance which intervend by people who clearly are a threat for the community before incidents happen.
...
##### 1. Heatmap graph representing "Firearms Provisions in US States" dataset
  For each state and year, the percentage of policy measures are represented. Although a lot of states are improving their provisions, the mayority do not reach even a half of them. 
...
### Discussion and Future Work 
  ...
  The report clearly explains that there is a relationship between the gun-control policies applied in a state and the number of kills occured. The more restrictive this policies are, the less people die.
	...
	Considering the score created to rank the states by their application of law considering gun restrictions the states mentioned	obtain a score below 20 throught the years from 1991 till 2017. This includes Alaska 10 -> 5, Louisiana ~10, South Carolina 15 -> 10, slight improvement in Delaware, on the contrary Illinois, despite having higher score compared to the mentioned before, still has a high rate of incidents.(30 -> 50)
	Now taking a look at the states with the lowest amount of incidents are Hawaii, Arizona, Utah, Idaho, California. They all have an incident rate below 50 per 100k of inhabitants.  Hawaii(30 -> 60), Arizona(10), Utah(10), Idaho(5), California(50 -> 80).
	This questions the created score ranking. Nevertheless the score is created equaly across all laws available, this implies that all laws have an equal impact on the costumer, which it is certainly not the case. To improve the score and create a more reliable metric a weighted sum can be considered.
